export default function StageSection() {
  return (
    <section class="stage-bar" id="stage-section">
      <div class="cue">
        <input id="productName" class="cue-input" type="text" placeholder="输入产品名，例如：华为Nova16" />
        <button class="cue-go" id="oneClickBtn" type="button">
      ...
      <p class="cue-hint" id="cueHint">
        下面是示例分镜。输入产品名点「一键生成」：自动知乎搜索 → 抓取真实素材 → MiMo 生成分镜。
      </p>
      ...
      <div class="adv-panel" id="advPanel" hidden>
        <div class="field-grid">
          ...
          <label>
            <span>目标时长(秒)</span>
            <input id="targetDuration" type="number" min="30" max="240" step="5" value="90" />
          </label>
          <label>
            <span>视频平台</span>
            <select id="platform">
              <option>抖音 / 快手 9:16</option>
              <option>B站竖屏 9:16</option>
        ...
        <label class="checkbox-row" style="margin-top: 14px; margin-bottom: 14px;">
          <input id="taoAnimationToggle" type="checkbox" checked />
          <span>启用道法起片过场动画（一生二，二生三，三生万物）</span>
        </label>

        <div class="segmented" role="group" aria-label="真人出镜布局">
          <button class="segment active" data-layout="center" type="button">
            真人居中
          </button>
          <button class="segment" data-layout="left" type="button">
            真人左侧
          </button>
          <button class="segment" data-layout="pip" type="button">
            右下小窗
          </button>
          <button class="segment" data-layout="none" type="button">
            无真人
          </button>
        </div>
        ...
        <label class="textarea-label">
          <span>产品事实（可选，帮助 MiMo 更准）</span>
          <textarea id="factsInput" rows="4" />
        </label>

        <label class="textarea-label">
          <span>后端地址（可选，留空走同源 Cloudflare）</span>
          <input id="apiBase" type="url" placeholder="https://your-codespace-8000.app.github.dev" />
        </label>
      </div>
    </section>
  );
